...
/// Bounded-poll Then for the `db_connections_in_use` gauge — and for
/// any other sampled-gauge contract that needs "eventually" rather than
/// "right now" semantics. The gauge is updated by a 1-second poll task
/// (see `crates/foundry-app/src/main.rs:208-219`); a single-instant
/// scrape can sample a transient idle window. This step polls the
/// metrics endpoint up to `timeout_seconds` and asserts on the first
/// satisfying sample.
///
/// See `docs/feature/slice-6-scenario-hardening/distill/wave-decisions.md`
/// for the rationale (D1 — Option A re-wording over silent step-impl fix).
#[then(
    regex = r#"^the scrape body's "([^"]+)" sample is eventually greater than (\d+) within (\d+) seconds$"#
)]
async fn then_scrape_body_sample_is_eventually_greater_than(
    world: &mut FoundryWorld,
    metric_name: String,
    threshold: u64,
    timeout_seconds: u64,
    ...
    let sample = poll_until_sample(
        addr,
        &metric_name,
        |s: &MetricSample| s.value > threshold_f,
        timeout,
    )
    .await;
    // Record the satisfying snapshot in the world so any downstream
    // Then in the same scenario can read the captured body (none today
    // for this scenario, but symmetry with the one-shot When/Then path
    // keeps future cross-step assertions valid).
